...
export default class Project {
  ...
  getBuildTree() {
    ...
    this.rootBuilder = new Builder(buildPath, this, preseededAddons);
    let builders = [ ...this.builders.values() ];
    let trees = builders.map((builder) => builder.toTree());
    return new MergeTree(trees, { overwrite: true });
  }

  builderFor(dir) {
    if (!this.builders.has(dir)) {
      this.builders.set(dir, new Builder(dir, this));
    }
    return this.builders.get(dir);
  }
  ...
  build(outputDir = 'dist') {
    ...
    let broccoliBuilder = new broccoli.Builder(this.getBuildTree());
    return broccoliBuilder.build()
      .then(() => {
        this._finishBuild(broccoliBuilder, outputDir);
      }).finally(() => {
  ...
  _finishBuild(broccoliBuilder, outputDir) {
    rimraf.sync(outputDir);
    copyDereferenceSync(broccoliBuilder.outputPath, outputDir);
    this._linkDependencies(path.join(this.dir, 'node_modules'), path.join(outputDir, 'node_modules'));
    if (this.printSlowTrees) {
      printSlowNodes(broccoliBuilder.outputNodeWrapper);
      ui.info(`Built - ${ Math.round(broccoliBuilder.outputNodeWrapper.buildState.totalTime) } ms @ ${ new Date().toString() }`);
    }
    if (this.audit) {
